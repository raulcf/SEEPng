...
public class InputAdapterFactory {
	...
	public static List<InputAdapter> buildInputAdapterOfTypeNetworkForOps(WorkerConfig wc, int streamId, List<UpstreamConnection> upc){
		List<InputAdapter> ias = new ArrayList<InputAdapter>();
	...
	public static List<InputAdapter> buildInputAdapterOfTypeHdfsForOps(WorkerConfig wc, int streamId, List<UpstreamConnection> upc){
		List<InputAdapter> ias = new ArrayList<InputAdapter>();
		short cType = upc.get(0).getConnectionType().ofType();
		Schema expectedSchema = upc.get(0).getExpectedSchema();
		if(cType == ConnectionType.BATCH.ofType()){
			LOG.info("Creating HDFS inputAdapter for upstream streamId: {} of type {}", streamId, ConnectionType.BATCH.withName());
			for(UpstreamConnection uc : upc){
				int opId = uc.getUpstreamOperator().getOperatorId();
				InputAdapter ia = new HdfsDataStream(opId,streamId,expectedSchema);
				ias.add(ia);
			}
		}
		return ias;
