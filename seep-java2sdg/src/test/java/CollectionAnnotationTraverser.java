...
public class CollectionAnnotationTraverser extends Traverser {

	public static void main(String[] args) throws CompileException, IOException {
		CollectionAnnotationTraverser poct = new CollectionAnnotationTraverser();

		String fileName = "./examples/Driver.java";
		// Parse each compilation unit.
		FileReader r = new FileReader(fileName);
		Java.CompilationUnit cu;
		try {
			cu = new CustomCollectionParser(new Scanner(fileName, r)).parseCompilationUnit();
		} finally {
			r.close();
		}
		...
		System.out.println("Class declarations:     "
				+ poct.classDeclarationCount);
		System.out.println("Interface declarations: "
				+ poct.interfaceDeclarationCount);
		System.out.println("Fields:                 " + poct.fieldCount);
		System.out
				.println("Local variables:        " + poct.localVariableCount);
	...
	public void traverseMethodDeclarator(Java.MethodDeclarator cu) {
		...
		System.out.println("line number for this method: "
				+ cu.getLocation().getLineNumber());
		if (cu.localVariables != null) {
			for (Entry<String, LocalVariable> entry : cu.localVariables
					.entrySet()) {
	...
	// Count annotations
	@Override
	public void traverseAnnotation(Java.Annotation an) {
	...
	// Count annotations
	@Override
	public void traverseMarkerAnnotation(Java.MarkerAnnotation an) {
	...
	// Count annotations
	@Override
	public void traverseNormalAnnotation(Java.NormalAnnotation an) {
	...
	// Count annotations
	@Override
	public void traverseSingleElementAnnotation(Java.SingleElementAnnotation an) {
	...
	public void traverseLocalVariableDeclarationStatement(
		...
		System.out.println("lineNumber: " + lvds.getLocation().getLineNumber()
				+ " type: " + lvds.type.toString());
		...
		if (lvds.localVariables != null) {
			for (Entry<String, LocalVariable> entry : lvds.localVariables
					.entrySet()) {
				System.out.println("name: " + entry.getKey());
				System.out.println("value: " + entry.getValue());
			}
