...
@Deprecated
public class SchedulePipelineCollector2 implements API {

	private int id;
	private ScheduleTask callbackTask;
	
	Map<Integer, OutputAdapter> streamIdToOutputAdapter;
	private OutputAdapter theOutputAdapter;
	...
	public SchedulePipelineCollector2(int id, List<OutputAdapter> outputAdapters, SeepTask task) {
		this.id = id;
	...
	public void sendAll(byte[] o) {
		...
		theOutputAdapter.sendAll(o);
	}

	@Override
	public void sendKey(byte[] o, int key) {
		if(NOT_SEND_API) throw new UnsupportedOperationException("Send API not defined, maybe this is a sink?");
		theOutputAdapter.sendKey(o, key);
	}
